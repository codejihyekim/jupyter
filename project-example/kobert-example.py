    ...
    for batch_id, (token_ids, valid_length, segment_ids, label) in enumerate(test_dataloader):
        ...
        for i in out:
            ...
            if np.argmax(logits) == 0:
                test_eval.append("공포스러우")
            elif np.argmax(logits) == 1:
                test_eval.append("놀라셨")
            elif np.argmax(logits) == 2:
                test_eval.append("분노가 나")
            elif np.argmax(logits) == 3:
                test_eval.append("슬프")
            elif np.argmax(logits) == 4:
                test_eval.append("중립이")
            elif np.argmax(logits) == 5:
                test_eval.append("행복하")
            elif np.argmax(logits) == 6:
                test_eval.append("혐오스러우")

        if test_eval[0] == '행복하':
            print(">> " + test_eval[0] + "시군요 "+ random.choice(happy_music) + "을/를 추천해드려요")
        elif test_eval[0] == '슬프':
            print(">> " + test_eval[0] + "시군요 "+ random.choice(sad_music) + "을/를 추천해드려요")
        else:
